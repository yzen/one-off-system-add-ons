...
XPCOMUtils.defineLazyGetter(this, "jawsesrStrings", () =>
  Services.strings.createBundle("chrome://jaws-esr/jawsesr.properties"));
...
async function checkVersionPromptAndDisableE10S() {
  ...
  const title = jawsesrStrings.formatStringFromName("jawsesr.dialog.title",
    [brandShortName], 1);
  const msg = jawsesrStrings.formatStringFromName("jawsesr.dialog.msg",
    [brandShortName, brandShortName], 1);
  const restartButtonText = jawsesrStrings.formatStringFromName("jawsesr.dialog.restart",
    [brandShortName], 1);
